...
# throughput 계산 (events_count / time_in_seconds)
FLINK_THROUGHPUT=$(python3 -c "print(round($EVENT_COUNT / ($FLINK_LATENCY_MS / 1000.0), 1) if $FLINK_LATENCY_MS > 0 else 0)")
SPARK_THROUGHPUT=$(python3 -c "print(round($EVENT_COUNT / ($SPARK_JOB_TIME_MS / 1000.0), 1) if $SPARK_JOB_TIME_MS > 0 else 0)")
...
log_info "Flink throughput:   ${FLINK_THROUGHPUT} events/sec"
log_info "Spark throughput:   ${SPARK_THROUGHPUT} events/sec"
log_info "=========================="
if [[ "$PASSED" == "true" ]]; then
...
printf '{"id":"%s","name":"%s","passed":%s,"events_count":%d,"flink_latency_ms":%d,"spark_job_time_ms":%d,"latency_ratio":%s,"flink_events_per_second":%s,"spark_events_per_second":%s,"flink_output_sha256":"%s","spark_output_sha256":"%s","elapsed_ms":%d}\n' \
    "$SCENARIO_ID" "$SCENARIO_NAME" "$PASSED" \
    "$EVENT_COUNT" "$FLINK_LATENCY_MS" "$SPARK_JOB_TIME_MS" "$LATENCY_RATIO" \
    "$FLINK_THROUGHPUT" "$SPARK_THROUGHPUT" \
